...
class ObjectiveFunction(object):
    def __init__(self, targets, types=2, alpha1=1, alpha2=0, beta1=1, beta2=0.5, threshold=0.9, coeff=(1, 1, 1), nperm=1):
    ...
    def get_fitness(self, x):
        ...
        random_cases = [[random.randint(0, self.types-1) for j in range(len(used))] for i in range(self.nperm)]
...
class HarmonySearch(object):
    def __init__(self, hms=30, hmv=25, hcmr=0.9, par=0.3, BW=0.2, threshold=0.9, coeff=(1, 1, 1), root_dir='./log', nperm=1, no_locals=1):
        
        self.threshold = threshold
        self.coeff = coeff
        self.no_locals = no_locals
        self._obj_fun = ObjectiveFunction(targets=targets, coeff=self.coeff, nperm=nperm)
    ...
    def _checkValidPosition(sel, position):
        if (position[0]>= lower[1][0] and position[1] >= lower[1][0] and position[0]<= upper[1][0] and position[1] <= upper[1][1]):
            return True
        # if(position[0] == -1 and position[1] == -1):
        #     return True
        return False

    def _proccess(self, position):
        if (position[0]>= lower[0][0] and position[1] >= lower[0][1] and position[0]<= upper[0][0] and position[1] <= upper[0][1]):
            return position
        else:
            return [-1., -1.]
    ...
    def _local_search(self):
        best_obj = float('-inf')
        best_local_harmony = None
        best_case = None
        best_fitness = None 
        for s in range(self.no_locals):
            # generate new harmony
            new_harmony = list()
            for i in range(0, self.size):
                if random.random() < self.hcmr:
            ...
            if new_fitness[0] > best_obj:
                best_obj = new_fitness[0]
                best_local_harmony = new_harmony
                best_fitness = new_fitness
                best_case = new_case
        
        return best_local_harmony, best_fitness, best_case
...
ax1.plot()
ax1.grid()
plt.savefig(os.path.join(root_dir, 'bestobj.png'))
...
plt.savefig(os.path.join(root_dir, 'bestcov.png'))
